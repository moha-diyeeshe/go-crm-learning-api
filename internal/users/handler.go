...
import (
	...
	"go-crm-learning-api/internal/auth" // Uses JWT manager to issue tokens after successful 2FA verify.
)
...
type Handler struct { // Holds service dependency for all HTTP endpoint methods.
	...
	jwtManager *auth.JWTManager // JWT manager used to issue bearer tokens after authentication flow.
}

func NewHandler(service *Service, jwtManager *auth.JWTManager) *Handler { // Constructor to build handler with service and JWT dependencies.
	return &Handler{service: service, jwtManager: jwtManager} // Returns pointer used by router registration.
}
...
func (h *Handler) RegisterRoutes(r chi.Router, authMiddleware func(http.Handler) http.Handler) { // Registers users routes and separates public auth flow from protected CRUD.
	...
	r.Group(func(protected chi.Router) { // Groups routes that require authenticated bearer token.
		...
		protected.Get("/", h.list)                                // Protected list endpoint.
		protected.Get("/{id}", h.get)                             // Protected get endpoint.
		protected.Put("/{id}", h.update)                          // Protected update endpoint.
		protected.Delete("/{id}", h.delete)                       // Protected delete endpoint.
		protected.Post("/{id}/password/change", h.changePassword) // Protected password change endpoint.
	})
}
