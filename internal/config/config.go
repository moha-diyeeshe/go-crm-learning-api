...
type Config struct {
	...
	JWTTTLMinutes      int    // Stores access-token expiration time in minutes.
	JWTRefreshTTLHours int    // Stores refresh-token expiration time in hours.
}
...
func Load() (Config, error) {
	...
	cfg := Config{ // Build config with defaults first, then validate required values.
		...
		JWTTTLMinutes:      getEnvAsInt("JWT_TTL_MINUTES", 60),        // Default access token TTL to 60 minutes.
		JWTRefreshTTLHours: getEnvAsInt("JWT_REFRESH_TTL_HOURS", 168), // Default refresh token TTL to 7 days.
	}
	...
	if cfg.JWTRefreshTTLHours <= 0 { // Ensure refresh TTL is a positive integer.
		return Config{}, fmt.Errorf("JWT_REFRESH_TTL_HOURS must be greater than zero") // Return explicit validation error.
	}
